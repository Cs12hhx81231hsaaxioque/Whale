...
#define TEST_SIZE_NUMS 5
static int test_send_size[TEST_SIZE_NUMS]={32,64,1024,16384,65536};
...
int main(int argc,char *argv[])
{
    ...
    else
    {
        ...
        // size_t total_length = sizeof(struct post_datagram);
        ...
        void* buff = malloc(65537);

        for (j=0; j<TEST_SIZE_NUMS; j++)
        {
            for (i=0; i<LOOP_COUNT;i++)
            {
                sge.length = test_send_size[j];
                write_task.done_flag=false;
                clock_gettime(CLOCK_MONOTONIC, &start);
                // memcpy(buff, sge.addr, sge.length);
                ...
                // asm volatile("" ::: "memory");
                while( *((volatile bool *)&(write_task.done_flag)) == false);
            ...
            ERROR_LOG("write [%d] avg time is [%ld] ns",test_send_size[j],  total_set_time/LOOP_COUNT);
            // for (i=0; i<50; i++)
